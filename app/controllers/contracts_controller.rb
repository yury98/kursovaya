class ContractsController < ApplicationController
  ...
  # Получение действующих контрактов
  def main
  ...
  # Запись в базу данных нового конракта
  def create
  ...
  # Обновление данных о контракте в базе данных
  def update
    @con.last_cb = current_user.fio
    if @con.update_attributes(con_params)
      @con.price_nds = @con.price * (@con.nds.to_i * 0.01)
      @con.price_name = RuPropisju.rublej(@con.price + @con.price_nds)
      # Проверка, не была ли исключена из контракта какая-либо услуга
  ...
  # Получение информации о действующих контрактах
  def all
